...
Override to `true` or `false` to force queue processors to run multiprocess or multithreaded.
Multiprocess scales much better, but requires more RAM; If empty, queue processors are run multiprocess on hosts with more than 3.5GiB of RAM, and multithreaded on hosts with less.
